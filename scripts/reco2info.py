...
def main(game_directory, reco_file, bagrada_game):
    """
    ...
    try:
        ...
        reco_tag.print_teams(teams, players, players_idx, dropped_players)
        ...
        reco_tag.print_combined_stats(
            reco_header, players, players_idx, teams, teams_idx, dropped_players, game_stats
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(f"Usage: python3 {sys.argv[0]} <game_directory> <reco_file> [<bagrada_game>]")
        sys.exit(1)
    
    game_directory = sys.argv[1]
    reco_file = sys.argv[2]
    if len(sys.argv) == 4:
        bagrada_game = sys.argv[3]
    else:
        bagrada_game = None

    try:
        main(game_directory, reco_file, bagrada_game)
